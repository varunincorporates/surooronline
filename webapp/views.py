...
def orderlist(request):
    ...
    if request.user.is_restaurant:
        orders = Order.objects.filter(r_id_id=request.user.restaurant.id).order_by('-timestamp')
        corders = [ ]
    else:
        orders = Order.objects.filter(orderedBy_id=request.user.id).order_by('-timestamp')
        corders = []
